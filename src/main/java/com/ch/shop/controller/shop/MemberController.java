...
public class MemberController {
	...
	@Autowired
	private RestTemplate restTemplate;//HTTP 요청능력+응답결과인 json을 자동으로 java 객체로 매핑(마치 jackson처럼) 
	...
	@ResponseBody //이 어노테이션을 설정하면, DispatcherServlet 은 jsp와의 매핑을 시도하지 않고, 반환값 그대로를 응답 정보로 보냄 
	public String getAuthUrl(@PathVariable("provider") String provider) throws Exception{
		...
		//로그인 절차가 성공할 수 있음
		//요청 시 지참할 파라미터 에는  clientId, callback url, scope...
		//https://accounts.google.com/o/oauth2/v2/auth?response_type=code&clientId=sdfjskdlfjsaklfjsa&scope=profile email
		StringBuffer sb = new StringBuffer();
		sb.append(oAuthClient.getAuthorizeUrl()).append("?")
		.append("response_type=code") // 이 요청에 의해 인가 code를 받을 것임을 알린다 
		.append("&client_id=").append(urlEncode(oAuthClient.getClientId())) //클라이언트 id추가 
	...
	public String handleGoogleCallback(String code, HttpSession session) {
		/*----------------------------------------------------------------
		 	구글이 보내온 인증 코드와 , 나의 client id, client Secret 를 조합하여, token을 요청하자!
		 	결국 개발자가 원하는 것은 사용자의 정보 이므로, 이 정보를 얻기 위해서는 토큰이 필요하므로..
		  ----------------------------------------------------------------*/
		...
		//이때, 구글과 같은 프로바이더와 데이터를 주고 받기 위해서는 HTTP 통신규약을 지켜서 말을 걸때는 머리,몸을 구성하여 요청을시도해야함
		...
		param.add("client_secret", google.getClientSecret());//클라이언 아이디 추가
		...
		//머리와 본문(몸)을 합쳐서 하나의 HTTP 요청 엔터티로 결합 
		HttpEntity<MultiValueMap<String, String>> request = new HttpEntity<>(param , headers);
		
		//구글에 요청 시작!!! , 스프링에서는 Http요청 후 그 응답 정보를 java객체와 자동으로 매핑해주는 편리한 객체를 지원해주는데, 
		//그 객체가 바로 RestTemplate (http요청 능력 + jack능력)
		...
		//회원정보 가져오기
		//구글에 요청을시도하려면 역시나 이번에도 Http프로토콜의 형식을 갖추어야 함 
		...
		Provider provider = provicerService.selectByName("google");//empy
		...
		//로그인에 성공하면, 브라우저를 종료할때까지는 자신의 정보를 접근할 수 있는 혜택을 부여해야 하므로, 
		//세션에 회원 정보를 담아둬야 한다..
		//jsp 의 내장객체 중 세션을 담당하는 내장객체명은 session 이고, 서블릿에서 자료형은 HttpSession 이다 .
		//jsp 의 내장객체 중 요청 정보를 담당하는 내장객체명은 request 이고, 서블릿에서 자료형은 HttpServletRequest 이다 .
	...
	public String handleNaverLogin(String code, HttpSession session) {
		...
		param.add("grant_type", "authorization_code");//임시코드를 이용하여 토큰을 요청하겠다는 것을 명시 
		param.add("code", code); // 구글로부터 발급받은 임시코드를 그대로 추가 
		param.add("client_id", client.getClientId());//클라이언 아이디 추가 
		param.add("client_secret", client.getClientSecret());//클라이언 아이디 추가
		...
		//토큰 달라고 요청하기 (구글, 네이버, 카카오, 마타 등 거의 모든 프로바이더가 토큰을 포함한 응답정보의 내용이 같다..)
		...
		log.debug("고유id={}", naverUser.getId());
		...
		//select * from provider where provider_name='google'
		Provider provider = provicerService.selectByName(client.getProvider());
		member.setProvider(provider);
		memberService.registOrUpdate(member);
